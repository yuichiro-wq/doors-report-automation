/**
 * DOORS Monthly Production Finalizer v1.6.2
 *
...
function finalizeDoorsMonthlyProductionV16(reportMonth) {
  ...
  doorsFinalizerSetShapeTextV16_(byId[cfg.keywordSlideId], cfg.keywordTitleId,
    '新規提案キーワード｜' + parts.year + '年' + parts.month + '月');
  doorsFinalizerSetShapeTextV16_(byId[cfg.keywordSlideId], cfg.keywordSubtitleId,
  ...
  return {status:'SUCCESS',version:cfg.version,reportMonth:reportMonth,lp:lp,
    keywordNoteTransformRepair:transformRepair,chartSync:chartSync,slideCount:afterSlides.length};
}
...
function diagnoseDoorsMonthlyProductionFinalizerV16(reportMonth) {
  ...
  try{lpData=doorsFinalizerBuildLpInsightV16_(ss,reportMonth);}catch(e){lpError=String(e&&e.message?e.message:e);}
  var checks={
    panelPresent:!!panel,
  ...
  return {status:failed.length?'BLOCK':'PASS',productionChanged:false,version:cfg.version,
    reportMonth:reportMonth,checks:checks,failed:failed,lpError:lpError,lp:lpData||null};
}
...
function doorsFinalizerSyncSessionsBarsV162_(slide){
  ...
  var max=Math.max.apply(null,values); if(!(max>0)) throw new Error('P.6 chart sync BLOCK: max sessions <= 0');
...
function doorsFinalizerSyncAiTrendV162_(slide){
  ...
  var yMin=Math.floor(rawMin/50)*50,yMax=Math.ceil(rawMax/50)*50;
  if(rawMin===yMin)yMin=Math.max(0,yMin-50); if(rawMax===yMax)yMax+=50; if(yMax-yMin<150)yMax=yMin+150;
  var top=2450000,bottom=4450000,x0=3300000,dx=700000,intrinsic=3000000;
  function yFor(v){return top+(yMax-v)/(yMax-yMin)*(bottom-top);}
  var ys=values.map(yFor);
  lineIds.forEach(function(id,i){var y1=ys[i],y2=ys[i+1];
  ...
  valueIds.forEach(function(id,i){doorsFinalizerSetElementTransformV162_(doorsFinalizerGetElementV16_(slide,id),
    {translateY:Math.max(top-120000,ys[i]-210000)});});
  var tickValues=[yMax,yMax-(yMax-yMin)/3,yMax-2*(yMax-yMin)/3,yMin];
  tickIds.forEach(function(id,i){var v=tickValues[i];doorsFinalizerSetShapeTextV16_(slide,id,String(Math.round(v)));
    doorsFinalizerSetElementTransformV162_(doorsFinalizerGetElementV16_(slide,id),{translateY:yFor(v)-60000});});
  return {status:'PASS',values:values,yMin:yMin,yMax:yMax};
}
...
function doorsFinalizerSyncReadingBarsV162_(slide){
  ...
  var max=Math.max.apply(null,values); if(!(max>0)) throw new Error('P.14 chart sync BLOCK: max reading rate <= 0');
...
function doorsFinalizerShapeTextV162_(slide,id){return String(doorsFinalizerGetElementV16_(slide,id).asShape().getText().asString()||'').trim();}
...
function doorsFinalizerBuildLpInsightV16_(ss,reportMonth){
  var cfg=DOORS_MONTHLY_FINALIZER_V16,prevMonth=doorsFinalizerPrevMonthV16_(reportMonth),sh=ss.getSheetByName(cfg.gscPageSheet);
  if(!sh)throw new Error('Missing sheet: '+cfg.gscPageSheet); var values=sh.getDataRange().getDisplayValues();
  if(values.length<2)throw new Error('GSC page SSOT is empty.'); var h=values[0];
  ...
  values.slice(1).forEach(function(r){var m=String(r[cMonth]||'').trim(),url=String(r[cPage]||'').trim();if(!url)return;
    ...
    if(m===prevMonth)prevByUrl[url]=obj;if(m===reportMonth&&url.indexOf('/doors/contents/')>=0&&isFinite(obj.clicks))current.push(obj);});
  current.sort(function(a,b){return a.rank-b.rank;});current=current.slice(0,10);if(current.length!==10)throw new Error('Expected 10 current /contents/ GSC pages; got '+current.length);
  ...
  var totalDelta=current.reduce(function(s,x){return s+x.delta;},0),declines=current.filter(function(x){return x.delta<0;}),gains=current.filter(function(x){return x.delta>0;});
  var pool=(totalDelta<0?declines:gains).slice().sort(function(a,b){return Math.abs(b.delta)-Math.abs(a.delta);}).slice(0,3);
  var denom=(totalDelta<0?declines:gains).reduce(function(s,x){return s+Math.abs(x.delta);},0),topAbs=pool.reduce(function(s,x){return s+Math.abs(x.delta);},0),share=denom?topAbs/denom:0;
  var approxTenth=Math.max(0,Math.min(10,Math.round(share*10))),jpMonth=Number(reportMonth.slice(5,7)),jpPrev=Number(prevMonth.slice(5,7)),title,insight;
...
function doorsFinalizerWriteLpTableV16_(slide,lp){
  ...
  var headers=['ページ',lp.currentMonthLabel+'Click',lp.prevMonthLabel+'Click','差',lp.currentMonthLabel+'Imp','CTR','順位'];for(var c=0;c<7;c++)table.getCell(0,c).getText().setText(headers[c]);
  lp.rows.forEach(function(x,i){var r=i+1,prevPos=x.prevPosition,posDelta=(isFinite(prevPos)&&isFinite(x.position))?prevPos-x.position:null,posText=doorsFinalizerFixedV16_(x.position,2)+'位';
    if(posDelta!=null&&Math.abs(posDelta)>=0.005)posText+='\n'+(posDelta>0?'↑':'↓')+Math.abs(posDelta).toFixed(2);
    ...
    for(var cc=0;cc<7;cc++)table.getCell(r,cc).getText().setText(vals[cc]);doorsFinalizerSetDeltaCellFillV16_(table.getCell(r,3),x.delta);doorsFinalizerSetDeltaCellFillV16_(table.getCell(r,6),posDelta==null?0:posDelta);});
}
...
function doorsFinalizerSetShapeTextV16_(slide,id,text){doorsFinalizerGetElementV16_(slide,id).asShape().getText().setText(String(text));}
